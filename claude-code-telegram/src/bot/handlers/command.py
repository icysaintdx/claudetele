...
async def new_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Also clear conversation history from message storage
    claude_integration: ClaudeIntegration = context.bot_data.get("claude_integration")
    if claude_integration and claude_integration.message_storage and old_session_id:
        try:
            deleted = await claude_integration.message_storage.delete_session_messages(
                old_session_id
            )
            logger.info(
                "Cleared conversation history for /new command",
                user_id=user_id,
