...
class Settings(BaseSettings):
    ...
    # Claude settings
    use_local_bun: bool = Field(
        False,
        description="Use local Bun-based Claude Code implementation instead of official SDK",
    )
    local_bun_project_path: Optional[Path] = Field(
        None,
        description="Path to local Claude Code Bun project (containing src/entrypoints/cli.tsx). "
    ...
    @field_validator("local_bun_project_path", mode="before")
    @classmethod
    def validate_local_bun_project_path(cls, v: Any) -> Optional[Path]:
        """Validate local Bun project path if provided."""
        if not v:
            return None
        if isinstance(v, str):
            value = v.strip()
            ...
            v = Path(value)
        if not v.exists():
            raise ValueError(f"Local Bun project path does not exist: {v}")
        if not v.is_dir():
            raise ValueError(f"Local Bun project path is not a directory: {v}")
        # Check for required entrypoint
        entrypoint = v / "src" / "entrypoints" / "cli.tsx"
        if not entrypoint.exists():
            raise ValueError(
                f"Local Bun project missing required entrypoint: {entrypoint}. "
                "Ensure this is a valid claude-code-local project."
            )
        return v  # type: ignore[no-any-return]
    ...
    def validate_cross_field_dependencies(self) -> "Settings":
        ...
        # Check local Bun requirements
        if self.use_local_bun:
            # When using local Bun, we don't need anthropic_api_key or CLI auth
            # But we should ensure the project path is set or can be auto-detected
            if self.local_bun_project_path and not self.local_bun_project_path.exists():
                raise ValueError(
                    "local_bun_project_path does not exist: "
                    f"{self.local_bun_project_path}"
                )
