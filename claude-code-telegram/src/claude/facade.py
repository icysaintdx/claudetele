...
from ..storage.message_storage import MessageHistoryStorage
...
class ClaudeIntegration:
    ...
    def __init__(
        ...
        local_manager: Optional[LocalBunManager] = None,
        ...
        Args:
            ...
            sdk_manager: Optional SDK manager for official Claude CLI
            local_manager: Optional local Bun manager for custom implementation
            session_manager: Session manager for persistence
            message_storage: Optional message storage for conversation history
        """
        self.config = config
        self.message_storage = message_storage

        # Initialize the appropriate manager based on configuration
        if config.use_local_bun:
            logger.info("Using local Bun-based Claude Code implementation")
            self.manager: Union[ClaudeSDKManager, LocalBunManager] = (
                local_manager
                or LocalBunManager(
                    config,
            ...
            logger.info("Using official Claude Code SDK")
    ...
    async def _execute(
        ...
        """Execute command via configured manager (SDK or Local Bun)."""
    ...
    async def start_new_session(
        ...
        """Start a new session, optionally clearing conversation history.
        ...
        Returns:
            The new session ID or None if session manager not available
        """
        logger.info(
        ...
        # Find and remove existing session for this user+project
        sessions = await self.session_manager._get_user_sessions(user_id)
        for session in sessions:
            if session.project_path == working_directory:
                # Remove old session
                await self.session_manager.remove_session(session.session_id)

                # Also clear conversation history if available
                if self.message_storage:
                    try:
                        deleted = await self.message_storage.delete_session_messages(
                            session.session_id
                        )
                        logger.info(
                ...
                logger.info(
                    "Removed old session for new session",
