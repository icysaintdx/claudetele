...
// Custom model profiles for non-Anthropic backends (Ollama adapter)
const CUSTOM_MODEL_PROFILES: Record<string, { url: string; model: string; key?: string; label: string }> = {
  'ollama': { url: 'http://192.168.50.115:11434/v1', model: 'huihui_ai/qwen3.5-abliterated:27b-Claude', label: 'Ollama - qwen3.5:27b (Local)' },
  'kimi': { url: 'https://api.860256.xyz/v1', model: 'kimi-k2.5', key: 'passw0rd', label: 'Kimi K2.5 (Remote)' },
  'gpt54': { url: 'https://api.860256.xyz/v1', model: 'gpt-5.4', key: 'passw0rd', label: 'GPT-5.4 (Remote)' },
  'gpt5.4': { url: 'https://api.860256.xyz/v1', model: 'gpt-5.4', key: 'passw0rd', label: 'GPT-5.4 (Remote)' },
  'sonnet45': { url: 'http://43.162.106.240:8080/v1', model: 'claude-sonnet-4.5', key: 'icysaintdx', label: 'Claude Sonnet 4.5 (Remote)' },
  'sonnet4.5': { url: 'http://43.162.106.240:8080/v1', model: 'claude-sonnet-4.5', key: 'icysaintdx', label: 'Claude Sonnet 4.5 (Remote)' },
}

function switchCustomModel(profileKey: string): string | null {
  const profile = CUSTOM_MODEL_PROFILES[profileKey.toLowerCase()]
  if (!profile) return null
  process.env.OLLAMA_BASE_URL_RUNTIME = profile.url
  process.env.OLLAMA_MODEL_RUNTIME = profile.model
  process.env.OLLAMA_API_KEY_RUNTIME = profile.key || ''
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  // Handle custom model profiles first (no Anthropic validation needed)
  if (args) {
    const customLabel = switchCustomModel(args);
    if (customLabel) {
      onDone(`Switched to ${chalk.bold(customLabel)}. Next message will use the new model.`);
      return;
    }
  }

  // Show available custom models when no args
  if (!args) {
    const currentModel = process.env.OLLAMA_MODEL_RUNTIME || process.env.OLLAMA_MODEL || '(default)';
    const lines = [
      chalk.bold('Available models:'),
      '',
      ...Object.entries(CUSTOM_MODEL_PROFILES)
        .filter(([k]) => !k.includes('.'))  // skip aliases like gpt5.4
        .map(([key, p]) => {
          const active = p.model === currentModel ? ' ◀' : '';
          return `  ${chalk.cyan(key.padEnd(12))} ${p.label}${active}`;
        }),
      '',
      `Usage: ${chalk.cyan('/model <name>')}`,
      `Example: ${chalk.cyan('/model sonnet45')}`,
    ];
    onDone(lines.join('\n'), { display: 'system' });
    return;
  }
